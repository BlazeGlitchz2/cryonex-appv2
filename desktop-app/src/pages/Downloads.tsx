...
export function Downloads() {
  ...
  const confirmInstall = async () => {
    ...
    try {
      ...
      setApps(
        apps.map((a) =>
          a.id === selectedApp.id ? { ...a, isInstalled: true } : a,
        ),
      );
    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {apps.map((app) => (
          <div
            key={app.id}
            className="bg-gray-800 p-6 rounded-xl border border-gray-700 flex items-center justify-between group hover:border-blue-500/50 transition"
          >
