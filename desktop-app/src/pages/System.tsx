...
export function System() {
    ...
    const confirmOptimize = async () => {
        ...
        try {
            // Mock optimization
            await invoke("install_program", { name: actionType === "ram" ? "RAM Optimizer" : "Disk Cleaner" });
    ...
    if (!info) return (
        <div className="p-8 text-white flex items-center justify-center h-full">
            <div className="animate-pulse">Loading system info... (Ensure backend is running)</div>
        </div>
    );
        ...
        <div className="p-8 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="bg-gray-800 p-6 rounded-xl border border-gray-700">
                    ...
                    <div className="text-3xl font-bold text-white">{info.cpu_usage.toFixed(1)}%</div>
                    <div className="w-full bg-gray-700 h-2 rounded-full mt-4 overflow-hidden">
                        <div className="bg-blue-500 h-full transition-all duration-500" style={{ width: `${info.cpu_usage}%` }} />
                    </div>
                </div>
                ...
                <div className="bg-gray-800 p-6 rounded-xl border border-gray-700">
                    ...
                    <div className="text-3xl font-bold text-white">{formatBytes(info.used_memory)}</div>
                    <div className="text-sm text-gray-400">of {formatBytes(info.total_memory)}</div>
                    <div className="w-full bg-gray-700 h-2 rounded-full mt-4 overflow-hidden">
                        <div className="bg-purple-500 h-full transition-all duration-500" style={{ width: `${(info.used_memory / info.total_memory) * 100}%` }} />
                    </div>
                </div>
                ...
                {info.disks.map((disk, i) => (
                    <div key={i} className="bg-gray-800 p-6 rounded-xl border border-gray-700">
                        <div className="flex items-center gap-3 mb-4 text-yellow-400">
                            <HardDrive size={24} />
                            <h3 className="font-semibold text-white">{disk.name || "Local Disk"}</h3>
                        </div>
                        <div className="space-y-2">
                            ...
                            <div className="flex justify-between text-sm">
                                ...
                                <span className="text-white">{formatBytes(disk.available_space)}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                                <span className="text-gray-400">Total Space</span>
                                <span className="text-white">{formatBytes(disk.total_space)}</span>
                            </div>
                            <div className="w-full bg-gray-700 h-2 rounded-full mt-2 overflow-hidden">
                                <div className="bg-yellow-500 h-full" style={{ width: `${((disk.total_space - disk.available_space) / disk.total_space) * 100}%` }} />
                            </div>
                        </div>
                    </div>
                ))}
            ...
            <h2 className="text-xl font-bold text-white mt-8 mb-4">AI Optimization Suggestions</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {info.used_memory / info.total_memory > 0.8 ? (
                    <div className="bg-gray-800 p-6 rounded-xl border border-red-500/50 flex items-center justify-between">
                        <div>
                            <h3 className="font-semibold text-white mb-1">High Memory Usage</h3>
                            <p className="text-sm text-gray-400">RAM usage is above 80%. Consider freeing up memory.</p>
                        </div>
                        <button
                    ...
                    <div className="bg-gray-800 p-6 rounded-xl border border-green-500/30 flex items-center justify-between">
                        <div>
                            <h3 className="font-semibold text-white mb-1">Memory Optimized</h3>
                            <p className="text-sm text-gray-400">RAM usage is within healthy limits.</p>
                        </div>
                ...
                {info.disks.some(d => d.available_space < 10 * 1024 * 1024 * 1024) ? (
                    <div className="bg-gray-800 p-6 rounded-xl border border-yellow-500/50 flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-sm text-gray-400">Less than 10GB free on one or more drives.</p>
                        </div>
                        <button
                    ...
                    <div className="bg-gray-800 p-6 rounded-xl border border-green-500/30 flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-sm text-gray-400">Sufficient free space available.</p>
                        </div>
                        <div className="text-green-400 text-sm font-medium">Good</div>
                    </div>
                )}
