...
class OfflineLLMService {
  ...
  constructor() {
    useOfflineModelStore.getState().setMode(this.isNative ? "native" : "web");
  }
  ...
  hasCachedModel(): boolean {
    if (this.isNative)
      return !!localStorage.getItem("cryonex_native_model_path");
  ...
  async preload(): Promise<boolean> {
    if (this.isNative) {
      try {
        const { cachedTier } = nativeLLM.getStatus();
        await nativeLLM.initialize(
          getEffectiveOfflineModelTier(undefined, cachedTier, "tiny"),
        );
        return true;
      } catch {
        return false;
  ...
  async runDiagnostics(): Promise<WebGpuDiagnostics> {
    ...
    try {
      ...
      diagnostics.adapterName =
        info.device || info.vendor || "Generic WebGPU Adapter";
      diagnostics.isSupported = true;
  ...
  async initialize(forceRedownload = false) {
    // Route to Native
    if (this.isNative) {
      const { cachedTier, currentTier } = nativeLLM.getStatus();
      return nativeLLM.initialize(
        getEffectiveOfflineModelTier(undefined, cachedTier, currentTier),
        forceRedownload,
      );
    }
    ...
    const { setDownloading, setProgress, setInitialized, setError } =
      useOfflineModelStore.getState();

    // WebGPU Support Check
    if (!navigator.gpu) {
      const errorMsg =
        "WebGPU is not supported on this device. Offline mode is unavailable.";
      setError(errorMsg);
