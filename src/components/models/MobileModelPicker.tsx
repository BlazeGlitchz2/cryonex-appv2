...
export function MobileModelPicker({ open, onOpenChange, type = "text" }: MobileModelPickerProps) {
    ...
    const categories = [
        ...
        { id: "reasoning" as ModelCategory, label: "Reasoning", icon: Cpu },
    ];
    ...
    return (
        <Sheet open={open} onOpenChange={onOpenChange}>
            <SheetContent
                side="bottom"
                className="h-[85vh] rounded-t-3xl bg-[#0a0a0a] border-t border-white/10 p-0 flex flex-col"
            >
                {/* Drag Handle */}
                ...
                {/* Header */}
                <SheetHeader className="px-5 pb-4 space-y-4">
                    ...
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full h-12 pl-12 pr-4 rounded-2xl bg-white/5 border border-white/5 text-white placeholder:text-white/30 text-base focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-transparent transition-all"
                        />
                    </div>
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2 mb-1">
                                            <span className={`font-semibold text-base truncate ${isActive ? "text-white" : "text-white/90"
                                                }`}>
                                                {model.name}
                                            </span>
                                            {model.showcase && (
                                                <span className="px-2 py-0.5 rounded-full bg-yellow-500/20 text-yellow-500 text-[10px] font-bold flex items-center gap-1">
                ...
                {/* View All Footer */}
                <div className="px-4 py-4 border-t border-white/5 bg-black/40 safe-bottom">
                    <button
                        onClick={() => {
                            onOpenChange(false);
                            useChatStore.getState().setModelBrowserOpen(true);
                        }}
                        className="w-full flex items-center justify-center gap-2 py-4 rounded-2xl bg-white/5 text-white/70 font-medium active:scale-[0.98] transition-transform"
                    >
