...
function InteractiveCore({
  ...
  const currentTheme = (colors as any)[
    theme === "liquid" ? "liquid" : "cosmic"
  ];
  ...
  useFrame((state) => {
    ...
    if (materialRef.current) {
      ...
      const targetColor = new THREE.Color(
        isStreaming ? baseColor : finishedColor,
      );
...
export default function CryonexLogo({
  isStreaming = false,
  className,
  scale = 1,
}: CryonexLogoProps) {
  ...
  return (
    <div
      className={`relative z-10 flex items-center justify-center pointer-events-auto ${className || "w-full h-full"}`}
    >
      <Canvas
      ...
      >
        ...
        <group scale={scale}>
          <Float
            speed={isStreaming ? 4 : 2}
            rotationIntensity={isStreaming ? 1 : 0.5}
            floatIntensity={0.5}
          >
