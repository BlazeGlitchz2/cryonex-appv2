...
const fragmentShader = `
  ...
  void main() {
    ...
    float n3 = snoise(uv * 10.0 + vec2(time * 0.15, -time * 0.05));
    
    float finalNoise = n1 * 0.5 + n2 * 0.3 + n3 * 0.2;
    
    // Colors
    vec3 c1 = vec3(0.5, 0.0, 0.8); // Purple
    vec3 c2 = vec3(0.0, 0.2, 0.8); // Blue
    vec3 c3 = vec3(0.8, 0.0, 0.4); // Pink
    
    vec3 color = bg;
    color += c1 * smoothstep(0.2, 0.8, n1 + 0.5) * 0.4;
    color += c2 * smoothstep(0.2, 0.8, n2 + 0.5) * 0.4;
    color += c3 * smoothstep(0.2, 0.8, n3 + 0.5) * 0.3;
    
    // Stars
    float stars = snoise(uv * 50.0);
    if (stars > 0.98) {
        color += vec3(1.0) * (stars - 0.98) * 50.0;
    }
