...
export function FallingPattern({
  ...
  // Detect Android/EMUI to prevent flashing
  // Detect Mobile/Android/EMUI to prevent flashing and improve performance
  const isMobile = typeof window !== 'undefined' && window.innerWidth < 768;
  const isAndroid = typeof navigator !== 'undefined' && /android/i.test(navigator.userAgent);

  // On all phones (or Androids), disable the complex falling pattern
  if (isMobile || isAndroid) return (
    <div className={cn('relative h-full w-full p-1 bg-black', className)}>
