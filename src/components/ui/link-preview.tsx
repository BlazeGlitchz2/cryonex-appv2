        ...
        <>
            ...
            >
                <HoverCardPrimitive.Trigger
                    ...
                    className={cn("text-cyan-400 hover:text-cyan-300 underline decoration-cyan-400/30 hover:decoration-cyan-300/60 transition-all font-medium cursor-pointer", className)}
                    asChild
                >
                ...
                >
                    <AnimatePresence>
                        {isOpen && (
                            ...
                            >
                                <div className="relative z-50 w-[360px]"> {/* Increased width for "huge" feel */}
                                    {loading ? (
                                        <div className="flex items-center justify-center h-48 text-white/50 gap-2 bg-white/5">
                                            <Loader2 className="h-5 w-5 animate-spin" />
                                            <span className="text-xs uppercase tracking-widest">Loading preview...</span>
                                        </div>
                                    ) : (
                                        <div className="flex flex-col">
                                            ...
                                            {displayImage ? (
                                                <div className="relative h-52 w-full overflow-hidden bg-black/50 group">
                                                    ...
                                                    <div className="absolute top-3 left-3 flex items-center gap-2 bg-black/40 backdrop-blur-md px-2 py-1 rounded-full border border-white/10">
                                                        {displayLogo ? (
                                                            <img src={displayLogo} alt="" className="h-3 w-3 rounded-full" />
                                                        ) : (
                                            ...
                                            <div className="p-5 space-y-3 relative bg-gradient-to-b from-[#0A0A0B] to-[#0A0A0B]/95">
                                                ...
                                                <div className="pt-3 mt-2 border-t border-white/5 flex items-center justify-between">
                                                    <span className="text-[10px] text-white/30 truncate max-w-[200px] font-mono">{domain}</span>
