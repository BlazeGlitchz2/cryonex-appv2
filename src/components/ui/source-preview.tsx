...
export const SourcePreviewProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    const preFetch = useCallback((sources: SourceData[]) => {
        ...
        if (isVisible && activeSource) {
            ...
            if (cached) {
                ...
                fetchMetadata(activeSource.url).then(data => {
                    if (activeSource.url === activeSource.url) { // Check if still active
                        setMetadata(data);
                        setLoading(false);
                    }
                });
    ...
    const handleHoverStart = useCallback((data: SourceData, e: React.MouseEvent) => {
    ...
    return (
        <SourcePreviewContext.Provider value={{ handleHoverStart, handleHoverMove, handleHoverEnd, preFetch }}>
            {children}
            <AnimatePresence>
                        ...
                        <div className="bg-[#0a0a0a]/95 backdrop-blur-2xl border border-white/10 rounded-2xl overflow-hidden shadow-2xl shadow-black/80">
                            {loading && !metadata ? (
                                <div className="h-32 w-full flex flex-col items-center justify-center gap-3 bg-white/5">
                                    ...
                                    <span className="text-[10px] text-white/40 uppercase tracking-widest font-medium">Fetching Preview</span>
                                </div>
                            ) : (
                                <>
                                    {displayImage ? (
                                    ...
                                    <div className="p-4 space-y-3">
                                        <div className="flex items-center gap-2">
                                            {displayLogo ? (
                                                <img src={displayLogo} alt="" className="w-4 h-4 rounded-sm" />
                                            ) : (
...
export const SourceLink = ({ source, children, className }: { source: SourceData, children: React.ReactNode, className?: string }) => {
    const context = useContext(SourcePreviewContext)
    if (!context) return <a href={source.url} target="_blank" rel="noopener noreferrer" className={className}>{children}</a>
...
export const useSourcePreview = () => {
    const context = useContext(SourcePreviewContext);
    if (!context) throw new Error("useSourcePreview must be used within SourcePreviewProvider");
    return context;
};
