...
interface PromptInputBoxProps {
  ...
  onSubmit?: (text: string, files?: File[], enableSearch?: boolean, enableCanvas?: boolean) => void;
...
export const PromptInputBox = React.forwardRef<HTMLDivElement, PromptInputBoxProps>((props, ref) => {
  ...
  const { onSend, onSubmit, isLoading = false, placeholder = "Type your message here...", className, disabled = false } = props;
  ...
  const handleSubmit = () => {
    ...
    if (input.trim() || files.length > 0) {
      ...
      // Support both onSend and onSubmit for backward compatibility

      if (onSubmit) {

        // For onSubmit, pass the original input text and flags separately

        onSubmit(input.trim(), files, showSearch, showCanvas);

      } else if (onSend) {

        // For onSend, pass the formatted message with prefixes
  ...
  const handleStopRecording = (duration: number) => {
    ...
    const message = `[Voice message - ${duration} seconds]`;

    if (onSubmit) {

      onSubmit(message, [], false, false);
