...
export function PersonalizationWizard() {
  ...
  const [step, setStep] = useState<
    "country" | "school" | "grade" | "curriculum"
  >("country");
  ...
  useEffect(() => {
    if (
      ...
      !user.country &&
      !user.schoolId &&
      !sessionStorage.getItem("skipped_personalization")
    ) {
  ...
  return (
    <div className="fixed inset-0 z-[100] overflow-y-auto bg-black/80 p-4 backdrop-blur-xl">
      <motion.div
        initial={{ opacity: 0, scale: 0.95 }}
        animate={{ opacity: 1, scale: 1 }}
        exit={{ opacity: 0, scale: 0.95 }}
        className="mx-auto my-4 flex min-h-[min(500px,calc(100dvh-2rem))] w-full max-w-md flex-col overflow-hidden rounded-3xl border border-white/10 bg-[#0A0A0B] shadow-2xl"
      >
        {/* Progress Bar */}
              ...
              {step === "school" && (
                ...
                >
                  <div className="text-center mb-6">
                    ...
                    <h2 className="text-xl font-bold text-white">
                      Do you attend a specific school?
                    </h2>
                    <p className="text-sm text-white/50">
                      Select your school for tailored resources.
                    </p>
                  </div>
              ...
              {step === "curriculum" && (
                ...
                >
                  <div className="text-center mb-6">
                    ...
                    <p className="text-sm text-white/50">
                      We'll prioritize relevant study materials.
                    </p>
                  </div>
                  ...
                  <div className="space-y-2">
                    {country?.curriculums.map((c) => (
                      ...
                      >
                        {c}
                        {curriculum === c && (
                          <Check className="ml-auto h-4 w-4 text-pink-400" />
                        )}
                      </button>
                    ))}
                    <button
