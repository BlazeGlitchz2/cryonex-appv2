...
export function StudyDashboardHeader({
  ...
  return (
    <div className={cn("flex w-full flex-col", compact ? "gap-3" : "gap-5")}>
      <div
        className={cn(
          "flex flex-col gap-3",
          compact
      ...
      >
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 max-w-lg text-sm leading-6 text-foreground/52">
              {userName ? `Welcome back, ${userName}. ` : ""}
              Your study lanes and focus tools stay in one calm place.
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto]">
          <div className="relative group/search min-w-0">
            ...
            <input
              ...
              className="deepshi-panel w-full rounded-2xl border border-border/60 bg-foreground/[0.04] py-3 pl-10 pr-4 text-sm text-foreground placeholder:text-foreground/25 focus:outline-none focus:ring-1 focus:ring-white/10"
            />
          </div>
