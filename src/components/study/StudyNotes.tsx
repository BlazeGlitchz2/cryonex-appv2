          ...
          <div className="max-w-3xl mx-auto prose prose-invert prose-headings:text-purple-400 prose-strong:text-purple-300 prose-a:text-blue-400 prose-code:text-orange-300">
            <ReactMarkdown components={{
              a: (props: any) => {
                ...
                let domain = "";
                try { domain = new URL(url).hostname.replace("www.", ""); } catch (e) { domain = "Source"; }
                return (
                  <a target="_blank" rel="noreferrer" className="group inline-flex items-center gap-1.5 px-2 py-1 rounded-lg bg-white/5 border border-white/10 hover:bg-purple-500/10 hover:border-purple-500/30 transition-all no-underline mx-1 align-middle" {...props} >
                    <span className="text-[10px] text-white/30 group-hover:text-purple-400/50">{domain}</span>
                    <span className="text-xs text-white/70 group-hover:text-white truncate max-w-[150px]">{props.children}</span>
                  </a>
                );
              },
              ...
              h1: (props: any) => <h1 className="text-2xl font-bold text-white mt-8 mb-4 border-b border-white/10 pb-2" {...props} />,
              h2: (props: any) => <h2 className="text-xl font-semibold text-purple-400 mt-6 mb-3 flex items-center gap-2" {...props} />,
              ul: (props: any) => <ul className="list-disc list-outside ml-6 space-y-2 my-4 text-gray-300" {...props} />,
