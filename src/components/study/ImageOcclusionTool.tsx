...
import { Eye, EyeOff, Plus, Save, Sparkles, Trash2, Upload, MousePointer2 } from "lucide-react";
...
export function ImageOcclusionTool({ materialId }: { materialId?: Id<"studyMaterials"> }) {
    ...
    const existingOcclusions = useQuery(api.imageOcclusion.listOcclusions, materialId ? { materialId } : "skip");
    ...
    const handleAutoDetect = async () => {
        ...
        try {
            const detected = await detectLabels({ storageId });
            setMasks([...masks, ...detected.map((d: any) => ({ ...d, isHidden: true }))]);
    ...
    const toggleMask = (id: string) => {
        setMasks(masks.map(m => m.id === id ? { ...m, isHidden: !m.isHidden } : m));
    };
                            ...
                            >
                                ...
                                {!mask.isHidden && (
                                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                                        <span className="text-xs text-orange-500 font-bold drop-shadow-md">Reveal</span>
                                    </div>
                                )}
                            </div>
                        ))}
