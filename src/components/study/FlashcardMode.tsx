...
import { X, Check, RotateCcw, ChevronLeft, ChevronRight, GraduationCap, Clock, ThumbsUp, ThumbsDown, Zap } from "lucide-react";
...
export function FlashcardMode({ cards, onComplete, onClose }: FlashcardModeProps) {
        ...
        <div className="fixed inset-0 z-50 flex flex-col bg-[#050014] text-white">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-[#0A0A0B]/50 backdrop-blur-xl">
                ...
                <div className="flex flex-col items-center">
                    ...
                    <span className="text-xs text-white/50">{currentIndex + 1} / {cards.length}</span>
                </div>
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="absolute inset-0 backface-hidden rounded-3xl border border-white/10 bg-[#0A0A0B]/80 backdrop-blur-2xl shadow-2xl p-8 flex flex-col items-center justify-center text-center">
                                ...
                                <p className="absolute bottom-8 text-xs text-white/30 animate-pulse">Tap to flip</p>
                            </div>
                            ...
                            <div className="absolute inset-0 backface-hidden rotate-y-180 rounded-3xl border border-primary/20 bg-primary/5 backdrop-blur-2xl shadow-2xl p-8 flex flex-col items-center justify-center text-center">
                                ...
                                <p className="text-xl md:text-2xl font-medium leading-relaxed text-white/90">{currentCard.back}</p>
                            </div>
                        </motion.div>
                    </motion.div>
                </AnimatePresence>
                    ...
                    {!isFlipped ? (
                        ...
                        <div className="grid grid-cols-4 gap-3">
                            <div className="flex flex-col gap-1">
                                ...
                                <span className="text-[10px] text-center text-white/30">1 min</span>
                            </div>
                            <div className="flex flex-col gap-1">
                                <Button
                                    variant="outline"
                                ...
                                <span className="text-[10px] text-center text-white/30">10 min</span>
                            </div>
                            <div className="flex flex-col gap-1">
                                <Button
                                    variant="outline"
                                ...
                                <span className="text-[10px] text-center text-white/30">1 day</span>
                            </div>
                            <div className="flex flex-col gap-1">
                                <Button
                                    variant="outline"
                                ...
                                <span className="text-[10px] text-center text-white/30">4 days</span>
                            </div>
                        </div>
                    )}
                </div>
