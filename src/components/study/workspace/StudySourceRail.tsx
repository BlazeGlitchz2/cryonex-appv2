...
export function StudySourceRail({
  ...
  return (
    <div className="flex h-full flex-col bg-white text-slate-900 dark:bg-[#0d1117] dark:text-slate-50">
      <div className="border-b border-slate-200/80 px-4 py-5">
        <div className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-600 dark:border-white/10 dark:bg-white/5 dark:text-slate-300">
        ...
        <div className="mt-4 grid gap-2 text-xs text-slate-600">
          <div className="flex items-center gap-2">
            <BookText className="h-3.5 w-3.5 text-cyan-600 dark:text-cyan-300" />
        ...
        <div className="space-y-1.5">
          {sections.map((section, index) => (
            <button
              ...
              className={cn(
                "group flex w-full items-start gap-3 rounded-lg border px-3 py-2 text-left transition-all",
                "border-transparent hover:border-slate-200 hover:bg-white dark:hover:border-white/10 dark:hover:bg-white/5",
              )}
            >
              <span className={cn(
                "mt-0.5 flex h-6 w-6 items-center justify-center rounded-lg border text-[11px] font-semibold transition-colors",
                index === 0
        ...
        <div className="flex flex-col gap-2">
          {tools.map((tool) => (
            <Button
              ...
              className={cn(
                "h-10 justify-start rounded-lg border px-3 text-sm font-medium shadow-none transition-all",
                activeTool === tool.id
