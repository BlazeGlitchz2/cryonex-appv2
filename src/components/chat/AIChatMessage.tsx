            ...
            >
                ...
                <div className="absolute -top-3 right-0 z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <div className="flex items-center gap-1 p-1 rounded-full bg-black/60 backdrop-blur-xl border border-white/10 shadow-lg">
                        <button
                            onClick={() => setTextSize(prev => prev === "base" ? "lg" : prev === "lg" ? "xl" : "base")}
                        ...
                        >
                            ...
                            <span className="text-[7px] ml-0.5">{textSize === "base" ? "+" : textSize === "lg" ? "++" : "STD"}</span>
                        </button>
                    </div>
                </div>
                <div className={cn(
                    ...
                    style={{
                        fontSize: textSize === "lg" ? "1.125rem" : textSize === "xl" ? "1.25rem" : textSize === "sm" ? "0.875rem" : "1rem",
                        lineHeight: textSize === "lg" ? "1.75rem" : textSize === "xl" ? "1.85rem" : textSize === "sm" ? "1.25rem" : "1.5rem"
                    }}
                >
                    <ReactMarkdown
                        ...
                        components={{
                            ...
                            code({ node, inline, className, children, ...props }: any) {
                                ...
                                if (!inline && language === 'mermaid') {
                                    return <PremiumMermaid chart={String(children).replace(/\n$/, "")} />;
                                }

                                return !inline && match ? (
                                    <PremiumCodeBlock language={language}>{String(children).replace(/\n$/, "")}</PremiumCodeBlock>
                                ) : (
                                    <code className="rounded-md bg-white/10 px-1.5 py-0.5 font-mono text-sm text-cyan-200 border border-white/5" {...props}>
                                        {children}
                                    </code>
                                );
                            },
                            ...
                            blockquote: ({ children }) => {
                                ...
                                if (React.isValidElement(firstChild) && (firstChild.props as any).node?.tagName === 'p') {
                                    ...
                                    if (typeof text === 'string') {
                                        if (text.startsWith('[!INFO]')) return <PremiumCallout type="info">{children}</PremiumCallout>;
                                        if (text.startsWith('[!WARNING]')) return <PremiumCallout type="warning">{children}</PremiumCallout>;
                                        if (text.startsWith('[!DANGER]')) return <PremiumCallout type="danger">{children}</PremiumCallout>;
                                        if (text.startsWith('[!TIP]')) return <PremiumCallout type="tip">{children}</PremiumCallout>;
                                    }
                                }
                            ...
                            p: ({ children }) => {
                                const childrenArray = React.Children.toArray(children);
                                const images = childrenArray.filter(child =>
                                    React.isValidElement(child) &&
                                    (child.type === PremiumImage || ((child.props as any) && (child.props as any).node && (child.props as any).node.tagName === 'img'))
                                );

                                const hasOnlyImages = images.length > 0 && images.length === childrenArray.filter(c => typeof c !== 'string' || c.trim() !== '').length;
                            ...
                            ol: ({ children }) => <ol className="list-decimal list-outside ml-5 space-y-2 my-4 marker:text-slate-500">{children}</ol>,
                            ...
                            h3: ({ children }) => <h3 className="text-lg font-medium text-cyan-100 mb-2 mt-4">{children}</h3>,
