...
export default function App() {
  ...
  const handleSend = async (text: string, files?: File[]) => {
    ...
    const predictedModel =
      isImageIntent && !isExplicitNonImage ? "pollinations/flux" : activeModel;
    ...
    try {
      ...
      if (text.startsWith("[Think] "))
        systemInstruction =
          "You are in REASONING mode. You MUST output your internal thought process wrapped in <tool_call>...<tool_call> tags before your final response. The user wants to see how you think.";
      else if (text.startsWith("[Search] "))
        systemInstruction =
          "You are in SEARCH mode. Please provide a comprehensive answer with sources if possible.";
      else if (text.startsWith("[Canvas] "))
        systemInstruction =
          "You are in CANVAS mode. Focus on generating structured content, code, or designs.";
  ...
  const handleEditMessage = async (
    ...
    try {
      ...
      setTemporaryModel(
        isImageIntent && !isExplicitNonImage
          ? "pollinations/flux"
          : activeModel,
      );
      ...
      let systemInstruction = "";
      if (newContent.startsWith("[Think] "))
        systemInstruction =
          "You are in REASONING mode. You MUST output your internal thought process wrapped in <tool_call>...<tool_call> tags before your final response. The user wants to see how you think.";
      else if (newContent.startsWith("[Search] "))
        systemInstruction =
          "You are in SEARCH mode. Please provide a comprehensive answer with sources if possible.";
      else if (newContent.startsWith("[Canvas] "))
        systemInstruction =
          "You are in CANVAS mode. Focus on generating structured content, code, or designs.";
          ...
          <div className="mobile-scroll-x pb-2">
            {features.map((item, idx) => (
              <button
                key={idx}
                onClick={() =>
                  onSend(item.prompt || `Help me ${item.label.toLowerCase()}`)
                }
              ...
              >
                <div className="flex flex-col gap-3">
                  <div
                    className={`h-12 w-12 rounded-xl bg-gradient-to-br ${item.bgGradient} flex items-center justify-center`}
                  >
          ...
          {features.map((item, idx) => (
            ...
            >
              <div className="flex items-center gap-4">
                <div
                  className={`p-3 rounded-xl bg-gradient-to-br ${item.bgGradient} text-white shadow-inner`}
                >
                ...
                <div>
                  ...
                  <p className="text-xs text-white/50 group-hover:text-white/70 transition-colors uppercase tracking-wider">
                    {item.desc}
                  </p>
                </div>
              </div>
