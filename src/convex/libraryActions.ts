      ...
      try {
        ...
        if (response.ok) {
          ...
          if (content) {
            ...
            console.log(`[libraryActions] Used ${provider.name} for content enhancement`);
            break;
          }
        } else {
          console.warn(`[libraryActions] ${provider.name} failed: ${response.status}`);
        }
      } catch (error) {
