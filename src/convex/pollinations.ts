  ...
  handler: async (ctx, args) => {
    ...
    if (!key) {
      ...
      try {
        ...
        if (!response.ok)
          throw new Error(`Free tier failed: ${response.status}`);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Pollinations generation failed (${response.status}): ${errorText}`,
        );
      }
      ...
      console.error(
        "Pollinations Pro API generation error, falling back to free tier:",
        error,
      );
      ...
      try {
        ...
        console.error(
          "Free tier fallback fetch failed, returning hotlink:",
          fallbackError,
        );
        return freeUrl;
      }
