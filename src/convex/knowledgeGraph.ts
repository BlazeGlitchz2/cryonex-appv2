...
export const getGlobalGraph = query({
    ...
    handler: async (ctx) => {
        ...
        const addNode = (label: string, type: "concept" | "document", meta: any = {}) => {
        ...
        for (const map of mindMaps) {
            ...
            for (const edge of map.edges) {
                ...
                const sourceLabel = map.nodes.find((n: any) => n.id === edge.source)?.data.label;
                const targetLabel = map.nodes.find((n: any) => n.id === edge.target)?.data.label;
