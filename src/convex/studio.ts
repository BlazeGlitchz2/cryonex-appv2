...
export const executeCode = action({
  ...
  handler: async (ctx, args) => {
    ...
    const apiHostHeader =
      process.env.JUDGE0_API_HOST_HEADER || "judge0-ce.p.rapidapi.com";
          ...
          messages: [
            {
              role: "system",
              content:
                "You are Cryonex Studio AI, an expert coding assistant. Provide concise, correct code snippets based on the user's request. If code is provided, edit or explain it as requested.",
            },
            {
              role: "user",
