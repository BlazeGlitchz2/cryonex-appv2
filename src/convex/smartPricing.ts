...
export const FEATURE_COSTS: Record<string, number> = {
  ...
  deep_thinking: 0.1, // Extended reasoning with <think> tags
  web_search_lite: 0.2, // Basic web lookup
  file_processing: 0.15, // Processing attachments
};

// Study feature costs
export const STUDY_COSTS: Record<string, number> = {
  flashcards_10: 2.0, // Generate 10 flashcards
  flashcards_20: 3.5, // Generate 20 flashcards
  quiz_10: 3.0, // Generate 10-question quiz
  notes_generate: 2.5, // Generate study notes
  summary_generate: 1.5, // Generate summary
  simple_summary: 1.0, // Generate dyslexia-friendly summary
  podcast_script: 2.0, // Generate podcast script
  concept_map: 1.5, // Generate concept map
  full_study_pack: 12.0, // All study materials combined (discount)
  pdf_processing: 5.0, // PDF extraction and chunking
};

// Search costs
export const SEARCH_COSTS: Record<string, number> = {
  cached: 0.5, // Use cached results
  quick: 1.0, // Quick search, 3 sources
  standard: 3.0, // Standard search, 5 sources
  deep: 10.0, // Deep research, 10+ sources
};
...
export function getModelCost(model: string): number {
  ...
  if (lowerModel.includes("70b")) return 0.1;
  if (lowerModel.includes("8b")) return 0.05;
  if (lowerModel.includes("reasoner") || lowerModel.includes("-r1")) return 0.5;
  if (lowerModel.includes("opus")) return 1.5;
  if (lowerModel.includes("sonnet")) return 0.6;
  if (lowerModel.includes("haiku")) return 0.25;
  if (lowerModel.includes("gpt-4")) return 1.0;
...
export function calculateStudyCost(
  ...
  return (
    STUDY_COSTS[action as keyof typeof STUDY_COSTS] ||
    STUDY_COSTS["notes_generate"]
  );
}
...
export function getCostBreakdown(
  ...
  const total = Math.max(
    0.01,
    Math.round((base * multiplier + featureTotal) * 100) / 100,
  );
...
export function detectFeatures(
  ...
  if (
    lowerContent.includes("run this code") ||
  ...
  if (
    ...
    lowerContent.includes("search the web")
  ) {
