...
export const getStats = query({
    ...
    handler: async (ctx) => {
        ...
        const sessions = await ctx.db.query("sessions").filter(q => q.eq(q.field("isActive"), true)).collect();
...
export const getAllUsers = query({
    ...
    handler: async (ctx, args) => {
        ...
        let users = await ctx.db.query("users").order("desc").take(args.limit || 100);
...
export const getAllMessages = query({
    ...
    handler: async (ctx, args) => {
        ...
        let messages = await ctx.db.query("messages").order("desc").take(args.limit || 200);
...
export const getAllChats = query({
    ...
    handler: async (ctx, args) => {
        ...
        let chats = await ctx.db.query("chats").order("desc").take(args.limit || 100);
        ...
        const enrichedChats = await Promise.all(chats.map(async (chat) => {
            ...
            const messageCount = (await ctx.db.query("messages")
                .withIndex("by_chat", q => q.eq("chatId", chat._id))
                .collect()).length;
...
export const deleteMessage = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        await ctx.db.insert("auditLogs", {
            ...
            details: { reason: args.reason, content: message.content.substring(0, 100) },
            timestamp: Date.now(),
        });
...
export const deleteChat = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        await ctx.db.insert("auditLogs", {
            ...
            details: { reason: args.reason, title: chat.title, messageCount: messages.length },
            timestamp: Date.now(),
        });
