...
function calculateNextInterval(currentInterval: number, rating: number, reviewCount: number): number {
    const dayInMs = 24 * 60 * 60 * 1000;

    // First review
    if (reviewCount === 0) {
        switch (rating) {
            case 1: return 1 * dayInMs; // Wrong -> 1 day
            case 2: return 2 * dayInMs; // Hard -> 2 days
            case 3: return 4 * dayInMs; // Good -> 4 days
            case 4: return 7 * dayInMs; // Easy -> 7 days
            default: return 1 * dayInMs;
        }
    }
...
export const reviewFlashcard = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const now = Date.now();
        const currentInterval = card.lastReviewedAt && card.nextReviewDate
            ? card.nextReviewDate - card.lastReviewedAt
            : 24 * 60 * 60 * 1000; // Default 1 day if new

        const nextInterval = calculateNextInterval(currentInterval, args.rating, card.reviewCount || 0);
        ...
        return { nextReviewDate, intervalDays: Math.round(nextInterval / (24 * 60 * 60 * 1000)) };
    },
});
    ...
    handler: async (ctx) => {
        ...
        flashcards.forEach(card => {
            if (!card.nextReviewDate) {
                ...
                if (daysFromNow <= 7) { // Look ahead 7 days
