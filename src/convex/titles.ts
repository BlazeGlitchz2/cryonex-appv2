...
export const generateTitle = action({
    ...
    handler: async (ctx, args) => {
        ...
        if (!userMessage) {
            ...
            const messages = await ctx.runQuery((api as any).messages.list, { chatId: args.chatId });
        ...
        for (const provider of providers) {
            ...
            try {
                ...
                const response = await fetch(provider.url, {
                    ...
                    body: JSON.stringify({
                        ...
                        messages: [
                            {
                                role: "system",
                                content: "Generate a very short, concise title (max 5 words) for this chat based on the user's message. Do not use quotes. Do not be chatty. Just output the title."
                            },
                ...
                if (!response.ok) {
                    ...
                    console.error(`[Title] ${provider.name} failed: ${response.status} - ${errText}`);
                    continue;
                }
        ...
        await ctx.runMutation((api as any).chats.rename, { chatId: args.chatId, title });
    },
});
