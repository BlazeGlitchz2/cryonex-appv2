...
function detectGpuTier(): {
  ...
  try {
    ...
    if (/nvidia|geforce|rtx|gtx|radeon|rx\s?\d{4}/i.test(gpuInfoLower)) {
      console.log(
        "[GPU Detection] Dedicated GPU detected (fallback high):",
        gpuInfo,
      );
      return { tier: "high", gpuInfo };
    }
...
function calculateTier(metrics: PerformanceMetrics): PerformanceTier {
  const { gpuTier, deviceMemory, cpuCores, fps, deviceType, connectionType } =
    metrics;
...
export function usePerformance(): UsePerformanceResult {
  ...
  const runBenchmark = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        "[Performance Detection] Screen Diagonal:",
        screenDiagonal,
        "inches",
      );
      ...
      if (
        ...
        console.log(
          "[Performance Detection] Skipping FPS measurement (mobile/tablet or low-end)",
        );
      }
